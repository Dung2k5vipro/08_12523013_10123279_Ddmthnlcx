      ...
      <div className={styles.container}>
        <header className={styles.header}>
          <div className={styles.intro}>
            ...
            <p>Bài toán Regression trên dataset Fuel Consumption Ratings 1995–2023, so sánh 4 mô hình: Linear Regression, Decision Tree, KNN và SVR.</p>
          </div>
          <SystemStatus />
        </header>

        {fieldsError && <div className={styles.globalError}><ErrorAlert error={fieldsError} onDismiss={() => setFieldsError(null)} /><button type="button" onClick={retryFields}>Thử lại tải cấu hình</button></div>}

        <section className={styles.mainGrid} aria-label="Khu vực dự đoán">
          <div className={styles.panel}><div className={styles.panelHeading}><p className={styles.kicker}>Thông tin đầu vào</p><h2>Thông tin xe</h2></div><PredictionForm fieldsConfig={fieldsConfig} loading={predicting || Boolean(fieldsError)} serverErrors={serverErrors} onSubmit={handleSubmit} onReset={handleReset} /></div>
          <div ref={resultRef} className={styles.resultPanel} tabIndex={-1}><ErrorAlert error={predictionError} onDismiss={() => setPredictionError(null)} /><ResultCard result={result} /></div>
        </section>

        <StatsCards refreshKey={dataRefreshKey} />
        <HistoryTable key={dataRefreshKey} refreshKey={dataRefreshKey} />

        <footer className={styles.footer}>© 2026 Fuel Consumption Ratings · Dữ liệu dùng cho mục đích tham khảo.</footer>
      </div>
    </main>
  );
}
