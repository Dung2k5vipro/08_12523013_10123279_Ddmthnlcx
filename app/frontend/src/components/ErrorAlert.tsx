...
export default function ErrorAlert({ error, onDismiss }: ErrorAlertProps) {
  ...
  return (
    <section className={styles.alert} role="alert">
      <div><strong>{getErrorMessage(error)}</strong>{error.details.length > 0 && <ul>{error.details.map((detail) => <li key={`${detail.field}-${detail.message}`}>{detail.field}: {detail.message}</li>)}</ul>}<small>Mã lỗi: {error.code}{error.requestId ? ` · Request ID: ${error.requestId}` : ''}</small></div>
